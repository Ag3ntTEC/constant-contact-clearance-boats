...
Local development falls back to `.data/shared-history.json`, which is ignored by Git. Browser-local header image history from earlier versions is migrated into the shared store the first time the image history panel opens on that browser. Image history is keyed by normalized image URL, keeps the most recent usage details, and removes older duplicate records automatically.
